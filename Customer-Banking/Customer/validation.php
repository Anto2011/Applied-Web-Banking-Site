...
//echo $errorString;
	...
	if($firstName_error =="" && $gender_error=="" && $lastName_error ==""  && $dob_error=="" && $telephone_error=="" && $streetName_error=="" && $community_error=="" && $parish_error=="" && $userName_error=="" && $password_error==""  && $password2_error=="" && $email_error=="" && $agreeErr==""){
        $pdo=getpdo();
        $userType="customer";
        $accountNumber=createAccount($db);
        $attempts=3;
        $password=md5($password);
        $user1= new customer($gender,$attempts,$accountNumber,$userType,$firstName,$lastName,$age,$dob,$telephone,$streetName,$community,$parish,$userName,$password,$email,$pdo);
        $user1->createUser();
